...
### 前提
> 毎回のメンテナンスする時にプランナー側がいつも入力ミスを修正するためホットフェスタことが多いです。なぜ事前にチェックしないかと思っていました。プランナーがマスタデータを弄た後にチェックできるツールを提案しました。

### 仕様の分析
1. Excelのシートの絡みが複雑すぎるから、プランナーと一々確認ことが不可能です。
2. シート間の紐付きに通用タイプが幾つあります。例 AシートのBカラムの値はBシートのBカラム
を参照するとかカラムに重複値が存在するかなど通用チェックタイプです。
3. プランナーに自由に紐付きを弄らせるために、vue3.0を用いてウェブサイトを作成する。
4. 現在処理流れが影響されないようにし、マステデータを生成してからチェックします
